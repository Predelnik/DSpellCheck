...
public:
  using Arg::operator();
  template <class ActualArg, std::enable_if_t<!std::is_same<std::decay_t<ActualArg>, self>::value, int> = 0> overload_class(const ActualArg &arg) : Arg(arg) {}
};
...
template <typename Ret, typename... Args> struct wrapper_for<Ret (*)(Args...)> { using type = func_ptr_wrapper<Ret(Args...)>; };
