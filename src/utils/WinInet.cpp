...
namespace WinInet {
CreateWinInetHandle::operator WinInetHandle() const { return {m_agent, m_access_type, m_proxy_string}; }

WinInetUrlHandle::WinInetUrlHandle(HINTERNET h_internet, const wchar_t *url, std::wstring_view headers, DWORD flags, void *context) {
...
void WinInetHandle::set_connect_timeout(DWORD ms) { InternetSetOption(m_handle.get(), INTERNET_OPTION_CONNECT_TIMEOUT, &ms, sizeof(DWORD)); }

void WinInetHandle::set_send_timeout(DWORD ms) { InternetSetOption(m_handle.get(), INTERNET_OPTION_SEND_TIMEOUT, &ms, sizeof(DWORD)); }

void WinInetHandle::set_receive_timeout(DWORD ms) { InternetSetOption(m_handle.get(), INTERNET_OPTION_RECEIVE_TIMEOUT, &ms, sizeof(DWORD)); }
...
void WinInetUrlHandle::proxy_settings_changed() { InternetSetOption(m_handle.get(), INTERNET_OPTION_PROXY_SETTINGS_CHANGED, nullptr, 0); }
...
WinInetHandle::WinInetHandle(const wchar_t *agent, DWORD access_type, const wchar_t *proxy_string, const wchar_t *proxy_bypass, DWORD flags) {
